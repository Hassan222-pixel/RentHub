...
export default function UniversitiesPage() {
  ...
  const handleImageFileChange = async (
    ...
    try {
      ...
      console.error("University image upload error:", err);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!name || isNaN(latitude) || isNaN(longitude)) {
      setError("Please fill name, latitude and longitude correctly");
      setSaving(false);
      return;
    }

    try {
      const payload = { name, latitude, longitude, image: image || undefined };

      let res: Response;
      if (mode === "create") {
        res = await fetch("/api/dashboard/universities", {
          method: "POST",
        ...
        res = await fetch(
          `/api/dashboard/universities/${editingUni?._id ?? ""}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p>No universities yet. Click &quot;New University&quot; to add one.</p>
      ) : (
